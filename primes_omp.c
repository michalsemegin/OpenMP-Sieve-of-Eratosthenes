...
/*
 * Buduje tablicę sita dla liczb z zakresu [0, n].
 * primeArray[i] == 1 gdy i jest liczbą pierwszą, 0 wpp.
 */

static void build_prime_array(int n, int sqrtN, char *primeArray) {
...
static void build_prime_array2(int n, int sqrtN, char *primeArray) {
  memset(primeArray, 1, (sqrtN + 1) * sizeof(char));
  primeArray[0] = primeArray[1] = 0;
  for (int i = 2; (long long)i * i * i * i <= n; i++) {
    if (primeArray[i]) {
      for (int j = i * i; (long long)j * j <= n; j += i) {
        primeArray[j] = 0;
      }
    }
  }
...
long long k1_sequential_division(int m, int n) {
  ...
  build_prime_array(n, sqrtN, primeArray);
...
long long k3_sequential_sieve(int m, int n) {
  ...
  memset(result, 1, rangeLen);
  build_prime_array2(n, sqrtN, primeArray);
...
long long k3a_sequential_sieve_local(int m, int n, int blockSize) {
  ...
  for (int low = m; low <= n; low += blockSize) {
    ...
    for (int p = 2; (long long)p * p <= high; p++) {
      ...
      if (fm <= 1) {
        fm = p + p; // p jest pierwsze – pomijamy
      }
...
int main(int argc, char *argv[]) {
   ...
   */
  const int repeats = 10;    /* dla k3, k3a, k4, k4a, k5  */
  const int repeats_div = 5; /* dla k1, k2 – wolniejsze    */
